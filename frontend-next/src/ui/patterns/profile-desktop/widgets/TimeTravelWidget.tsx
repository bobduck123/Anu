    ...
    <div className="p-3 space-y-3" style={{ color: textColor }}>
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setShowUploadModal(true)}
          className="p-1 rounded hover:bg-[color:rgba(246,212,203,0.1)] transition-colors" title="Add your art">
      ...
      <div className="space-y-1">
        ...
        <input type="range" min={1440} max={1940} step={10} value={currentYear}
          ...
          className="w-full h-1 appearance-none bg-[color:rgba(246,212,203,0.2)] rounded-full cursor-pointer
            ...
            [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-[var(--color-foreground)] [&::-webkit-slider-thumb]:cursor-grab" />
        {currentPeriod && (
      ...
      {isLoading && (
        <div className="flex gap-2">
          {[0, 1].map(i => (
            <div key={i} className="flex-1 aspect-square rounded animate-pulse bg-[color:rgba(246,212,203,0.05)]" />
          ))}
        </div>
      )}
      ...
      {error && !isLoading && (
        <div className="text-center py-3">
          ...
          <button onClick={retry} className="text-[9px] px-2 py-1 rounded bg-[color:rgba(246,212,203,0.1)] hover:bg-[color:rgba(246,212,203,0.15)]">
      ...
      {!isLoading && allItems.length > 0 && (
        <div className="flex gap-2">
          {allItems.map((item, i) => {
            ...
            return (
              <div key={i} className="flex-1 rounded overflow-hidden bg-[color:rgba(246,212,203,0.05)] border border-[color:rgba(246,212,203,0.1)]">
      ...
      {/* Mini upload modal */}
      {showUploadModal && (
        <div className="absolute inset-0 z-10 bg-[color:rgba(30,2,39,0.5)] flex items-center justify-center p-3 rounded-xl">
          <div className="bg-[#1e0227] border border-[color:rgba(246,212,203,0.15)] rounded-lg p-3 w-full max-w-[260px] space-y-2">
            <div className="flex items-center justify-between">
            ...
            <input value={uploadTitle} onChange={e => setUploadTitle(e.target.value)} placeholder="Title"
              className="w-full text-[10px] px-2 py-1 rounded bg-[color:rgba(246,212,203,0.1)] border border-[color:rgba(246,212,203,0.1)] outline-none" />
            ...
            <button onClick={handleUploadSubmit} disabled={!uploadFile}
              className="w-full text-[9px] py-1.5 rounded bg-[#7c413c] hover:bg-[#7c413c] disabled:opacity-30 text-[var(--color-foreground)]">
              Upload
            </button>
          </div>
        </div>
