...
export function RetroPixelTemplate({ mode = 'full' }: Props) {
  ...
  return (
    <div
      ...
      style={{
        background: state.lightMode ? '#f6d4cb' : '#1e0227',
        fontFamily: '"Press Start 2P", "Courier New", monospace',
        color: state.lightMode ? '#1e0227' : '#f6d4cb',
      }}
    >
      {/* Scanline overlay */}
      {state.crtEnabled && (
        <div className="fixed inset-0 pointer-events-none z-50" style={{
          background: 'repeating-linear-gradient(0deg, rgba(30,2,39,0.12) 0px, rgba(30,2,39,0.12) 1px, transparent 1px, transparent 3px)',
        }} />
      )}

      {/* Header */}
      <div className="flex items-center justify-between px-3 py-2 border-b"
        style={{ borderColor: state.lightMode ? 'rgba(30,2,39,0.15)' : 'rgba(246,212,203,0.15)' }}>
        <div className="flex items-center gap-2">
          <Paintbrush className="w-4 h-4 text-[#7c413c]" />
          <span className="text-[10px] font-bold tracking-wider">PIXEL STUDIO</span>
        </div>
        <div className="flex items-center gap-1">
          <button onClick={() => dispatch({ type: 'SHOW_MODAL', modal: 'help' })} className="p-1.5 rounded hover:bg-[color:rgba(246,212,203,0.1)]" title="Help">
            <HelpCircle className="w-3.5 h-3.5" />
          </button>
          <button onClick={() => dispatch({ type: 'SHOW_MODAL', modal: 'export' })} className="p-1.5 rounded hover:bg-[color:rgba(246,212,203,0.1)]" title="Export">
            <FileDown className="w-3.5 h-3.5" />
          </button>
          <button onClick={() => importInputRef.current?.click()} className="p-1.5 rounded hover:bg-[color:rgba(246,212,203,0.1)]" title="Import">
          ...
          <button onClick={() => dispatch({ type: 'SHOW_MODAL', modal: 'newDoc' })} className="p-1.5 rounded hover:bg-[color:rgba(246,212,203,0.1)]" title="New">
            <FilePlus className="w-3.5 h-3.5" />
          </button>
          <button onClick={() => dispatch({ type: 'TOGGLE_THEME' })} className="p-1.5 rounded hover:bg-[color:rgba(246,212,203,0.1)]" title="Theme">
          ...
          {!isCompact && (
            <button onClick={() => document.documentElement.requestFullscreen?.().catch(() => {})}
              className="p-1.5 rounded hover:bg-[color:rgba(246,212,203,0.1)]" title="Fullscreen">
      ...
      <div className={`flex-1 flex ${isCompact ? 'flex-col' : 'flex-row'} overflow-hidden`}>
        ...
        <div className={`${isCompact ? 'flex flex-row gap-1 px-2 py-1 border-b overflow-x-auto' : 'flex flex-col gap-1 p-2 border-r w-12'}`}
          style={{ borderColor: state.lightMode ? 'rgba(30,2,39,0.1)' : 'rgba(246,212,203,0.1)' }}>
            ...
            return (
              <button key={t} onClick={() => dispatch({ type: 'SET_TOOL', tool: t })}
                className={`p-1.5 rounded transition-colors ${state.tool === t
                  ? (state.lightMode ? 'bg-[color:rgba(30,2,39,0.2)] ring-1 ring-black/30' : 'bg-[color:rgba(246,212,203,0.2)] ring-1 ring-white/30')
                  : 'hover:bg-[color:rgba(246,212,203,0.1)]'
                }`}
                title={TOOL_LABELS[t]}>
          ...
          <button onClick={() => { actions.undo(); showToast('Undo'); }} className="p-1.5 rounded hover:bg-[color:rgba(246,212,203,0.1)]" title="Undo (Ctrl+Z)">
            <Undo2 className="w-3.5 h-3.5" />
          </button>
          <button onClick={() => { actions.redo(); showToast('Redo'); }} className="p-1.5 rounded hover:bg-[color:rgba(246,212,203,0.1)]" title="Redo (Ctrl+Y)">
          ...
          {!isCompact && (
            <>
              ...
              <button onClick={() => { actions.copy(); showToast('Copied'); }} className="p-1.5 rounded hover:bg-[color:rgba(246,212,203,0.1)]" title="Copy"><Copy className="w-3.5 h-3.5" /></button>
              <button onClick={() => { actions.paste(); showToast('Pasted'); }} className="p-1.5 rounded hover:bg-[color:rgba(246,212,203,0.1)]" title="Paste"><ClipboardPaste className="w-3.5 h-3.5" /></button>
              <button onClick={() => { actions.cut(); showToast('Cut'); }} className="p-1.5 rounded hover:bg-[color:rgba(246,212,203,0.1)]" title="Cut"><Scissors className="w-3.5 h-3.5" /></button>
              <div className="h-px bg-current opacity-20 my-1" />
              <button onClick={() => { actions.flip('h'); showToast('Flipped H'); }} className="p-1.5 rounded hover:bg-[color:rgba(246,212,203,0.1)]" title="Flip H"><FlipHorizontal2 className="w-3.5 h-3.5" /></button>
              <button onClick={() => { actions.flip('v'); showToast('Flipped V'); }} className="p-1.5 rounded hover:bg-[color:rgba(246,212,203,0.1)]" title="Flip V"><FlipVertical2 className="w-3.5 h-3.5" /></button>
              <button onClick={() => { actions.rotate(-90); showToast('Rotated'); }} className="p-1.5 rounded hover:bg-[color:rgba(246,212,203,0.1)]" title="Rotate L"><RotateCcw className="w-3.5 h-3.5" /></button>
              <button onClick={() => { actions.rotate(90); showToast('Rotated'); }} className="p-1.5 rounded hover:bg-[color:rgba(246,212,203,0.1)]" title="Rotate R"><RotateCw className="w-3.5 h-3.5" /></button>
              <div className="h-px bg-current opacity-20 my-1" />
              <button onClick={() => dispatch({ type: 'SHOW_MODAL', modal: 'clear' })} className="p-1.5 rounded hover:bg-[color:rgba(246,212,203,0.1)] text-[#7c413c]" title="Clear"><Trash2 className="w-3.5 h-3.5" /></button>
            </>
          )}
        </div>
        ...
        <div className="flex-1 flex items-center justify-center overflow-auto p-4 relative"
          ...
          <div className="relative" style={{ width: displayW, height: displayH }}>
            ...
            {/* Selection box */}
            {state.selection && (
              <div className="absolute border-2 border-dashed border-[var(--color-foreground)] pointer-events-none"
                style={{
                  ...
                  boxShadow: '0 0 0 1px rgba(30,2,39,0.5)',
                }}
              />
            )}
          </div>
        ...
        {!isCompact && (
          <div className="w-56 border-l p-3 flex flex-col gap-3 overflow-y-auto"
            ...
            <div>
              ...
              <canvas ref={previewCanvasRef}
                ...
                style={{
                  ...
                  borderColor: state.lightMode ? 'rgba(30,2,39,0.15)' : 'rgba(246,212,203,0.15)',
                  aspectRatio: '1',
                }}
              />
            </div>
            ...
            <div>
              ...
              {palettes && (
                <select
                  ...
                  style={{ borderColor: state.lightMode ? 'rgba(30,2,39,0.2)' : 'rgba(246,212,203,0.2)' }}
                >
            ...
            {/* Active color */}
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded border" style={{ backgroundColor: state.color, borderColor: 'rgba(246,212,203,0.2)' }} />
            ...
            <div className="space-y-1.5">
              ...
              {state.transEnabled && (
                <div className="flex items-center gap-1 ml-5">
                  <div className="w-4 h-4 rounded border" style={{ backgroundColor: state.transColor, borderColor: 'rgba(246,212,203,0.2)' }} />
            ...
            <button onClick={() => { actions.applyPalette(currentPalette); showToast('Palette applied'); }}
              className="text-[9px] px-2 py-1.5 rounded border hover:bg-[color:rgba(246,212,203,0.1)] transition-colors"
              style={{ borderColor: state.lightMode ? 'rgba(30,2,39,0.2)' : 'rgba(246,212,203,0.2)' }}>
              Convert to Palette
            </button>
          </div>
        ...
        {isCompact && (
          <div className="flex items-center gap-1 px-2 py-1.5 border-t overflow-x-auto"
            style={{ borderColor: state.lightMode ? 'rgba(30,2,39,0.1)' : 'rgba(246,212,203,0.1)' }}>
            <div className="w-5 h-5 rounded border shrink-0" style={{ backgroundColor: state.color, borderColor: 'rgba(246,212,203,0.2)' }} />
      ...
      {toast && (
        <div className="fixed bottom-16 left-1/2 -translate-x-1/2 px-4 py-2 rounded-lg text-[10px] z-50"
          style={{ background: state.lightMode ? '#1e0227' : '#f6d4cb', color: state.lightMode ? '#f6d4cb' : '#1e0227' }}>
          {toast}
        </div>
      )}
      ...
      {state.showExportModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[color:rgba(30,2,39,0.6)]" onClick={() => dispatch({ type: 'HIDE_MODAL', modal: 'export' })}>
          <div className="rounded-xl p-6 max-w-xs w-full" onClick={e => e.stopPropagation()}
            style={{ background: state.lightMode ? '#f6d4cb' : '#1e0227', color: state.lightMode ? '#1e0227' : '#f6d4cb' }}>
            ...
            <div className="space-y-3">
              <div>
                ...
                <select value={exportFormat} onChange={e => setExportFormat(e.target.value as ExportFormat)}
                  ...
                  style={{ borderColor: 'rgba(124,65,60,0.3)' }}>
              ...
              <div className="flex gap-2">
                <button onClick={() => dispatch({ type: 'HIDE_MODAL', modal: 'export' })}
                  className="flex-1 text-[10px] px-3 py-2 rounded border" style={{ borderColor: 'rgba(124,65,60,0.3)' }}>
                  Cancel
                </button>
                <button onClick={() => { actions.doExport(exportFormat); showToast('Exported'); }}
                  className="flex-1 text-[10px] px-3 py-2 rounded bg-[#7c413c] text-[var(--color-foreground)] font-bold">
                  Export
                </button>
              </div>
            </div>
      ...
      {state.showNewDocModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[color:rgba(30,2,39,0.6)]" onClick={() => dispatch({ type: 'HIDE_MODAL', modal: 'newDoc' })}>
          <div className="rounded-xl p-6 max-w-xs w-full" onClick={e => e.stopPropagation()}
            style={{ background: state.lightMode ? '#f6d4cb' : '#1e0227', color: state.lightMode ? '#1e0227' : '#f6d4cb' }}>
            ...
            <div className="grid grid-cols-5 gap-2 mb-4">
              {CANVAS_SIZES.map(size => (
                <button key={size} onClick={() => { actions.resizeCanvas(size); showToast(`New ${size}x${size}`); }}
                  className={`text-[10px] px-2 py-2 rounded border transition-colors hover:bg-[color:rgba(246,212,203,0.1)]`}
                  style={{ borderColor: state.width === size ? '#7c413c' : 'rgba(124,65,60,0.3)' }}>
                  {size}
                </button>
              ))}
            </div>
            <button onClick={() => dispatch({ type: 'HIDE_MODAL', modal: 'newDoc' })}
              className="w-full text-[10px] px-3 py-2 rounded border" style={{ borderColor: 'rgba(124,65,60,0.3)' }}>
              Cancel
            </button>
          </div>
        </div>
      ...
      {state.showClearModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[color:rgba(30,2,39,0.6)]" onClick={() => dispatch({ type: 'HIDE_MODAL', modal: 'clear' })}>
          <div className="rounded-xl p-6 max-w-xs w-full" onClick={e => e.stopPropagation()}
            style={{ background: state.lightMode ? '#f6d4cb' : '#1e0227', color: state.lightMode ? '#1e0227' : '#f6d4cb' }}>
            ...
            <div className="flex gap-2">
              <button onClick={() => dispatch({ type: 'HIDE_MODAL', modal: 'clear' })}
                className="flex-1 text-[10px] px-3 py-2 rounded border" style={{ borderColor: 'rgba(124,65,60,0.3)' }}>Cancel</button>
              <button onClick={() => { actions.clearCanvas(); showToast('Cleared'); }}
                className="flex-1 text-[10px] px-3 py-2 rounded bg-[#7c413c] text-[var(--color-foreground)] font-bold">Clear</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Help Modal */}
      {state.showHelpModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[color:rgba(30,2,39,0.6)]" onClick={() => dispatch({ type: 'HIDE_MODAL', modal: 'help' })}>
            ...
            <div className="space-y-2 text-[9px]">
              {[
                ...
                <div key={key} className="flex justify-between">
                  <kbd className="px-1.5 py-0.5 rounded bg-[color:rgba(246,212,203,0.1)] font-mono">{key}</kbd>
                  <span className="opacity-60">{action}</span>
                </div>
              ))}
            </div>
            <button onClick={() => dispatch({ type: 'HIDE_MODAL', modal: 'help' })}
              className="w-full mt-4 text-[10px] px-3 py-2 rounded border" style={{ borderColor: 'rgba(124,65,60,0.3)' }}>Close</button>
          </div>
        </div>
      )}
    </div>
