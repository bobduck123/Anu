        ...
        <div className="min-w-0">
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-3">
            <AnuSurfacePanel tone="soft">
              <p className="text-[11px] uppercase tracking-[0.16em] text-slate-400">Field state</p>
            ...
            <AnuSurfacePanel tone="soft">
              ...
              <p className="mt-3 text-lg font-semibold text-white">{items.length} constellations</p>
            ...
            <AnuSurfacePanel tone="soft">
              <p className="text-[11px] uppercase tracking-[0.16em] text-slate-400">Fallback posture</p>
              <p className="mt-3 text-lg font-semibold text-white">{surfaceMode === 'starfield' ? 'Celestial first' : '2D explicit'}</p>
              <p className="mt-2 text-sm leading-6 text-slate-300/82">
  ...
  return (
    <>
      {surfaceMode === 'starfield' ? (
        ...
        <div className="fixed inset-0 z-[5] overflow-y-auto bg-[#02050c] pb-24 pt-28" style={{ isolation: 'isolate' }}>
              ...
              {loading ? (
                ...
                items.map((item) => (
                  <Link
                    ...
                    className="block rounded-[1.6rem] border border-white/10 bg-[linear-gradient(180deg,rgba(8,12,24,0.86),rgba(5,8,16,0.94))] p-5 text-white shadow-[0_24px_80px_-38px_rgba(0,0,0,0.95)] transition-transform hover:-translate-y-1"
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <h3 className="text-2xl text-white" style={{ fontFamily: 'var(--anu-type-display)' }}>
                          {item.name}
                        </h3>
                        <p className="mt-3 text-sm leading-6 text-slate-300/82">
                          {item.description || 'Regional coordination constellation'}
                        </p>
                        <div className="mt-4 flex flex-wrap gap-3 text-xs text-slate-300/74">
                          <span className="inline-flex items-center gap-1">
                            <Layers className="h-3.5 w-3.5 text-[#f1c57a]" />
                            {item.domain || 'general'}
                          </span>
                          {item.geoLabel ? (
                            <span className="inline-flex items-center gap-1">
                              <MapPin className="h-3.5 w-3.5 text-[#8dc7f3]" />
                              {item.geoLabel}
                            </span>
                          ) : null}
                        </div>
                      </div>
                      <span className="rounded-full border border-white/10 bg-white/[0.05] px-3 py-1 text-[10px] uppercase tracking-[0.16em] text-slate-100">
