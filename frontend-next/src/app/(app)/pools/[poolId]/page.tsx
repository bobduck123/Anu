    ...
    return (
      <div className="min-h-screen" style={{ backgroundColor: 'var(--color-background)' }}>
        <div className="max-w-5xl mx-auto px-4 md:px-8 pt-28 pb-20">
          ...
          <div className="p-6 rounded-xl border border-[color:rgba(224,177,21,0.32)] bg-[color:rgba(224,177,21,0.12)]">
            <p className="text-[var(--color-foreground)]">{error || 'Pool not found'}</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="max-w-5xl mx-auto px-4 md:px-8 pt-28 pb-20">
        ...
        <div className="mb-10">
          {pool.category && (
            <span className="inline-block text-xs font-medium uppercase tracking-wider px-2.5 py-1 rounded-full mb-3"
              style={{ backgroundColor: 'var(--color-institutional-light)', color: 'var(--color-foreground)' }}>
              {pool.category}
            </span>
          )}
          <h1 className="text-3xl md:text-4xl font-semibold text-[var(--color-foreground)] mb-3"
            style={{ fontFamily: 'var(--anu-type-display)' }}>
            {pool.name}
          </h1>
          {pool.description && (
            <p className="text-lg text-[color:rgba(246,212,203,0.84)] max-w-2xl">{pool.description}</p>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
          <div className="card-civic">
            ...
            <p className="text-2xl font-semibold text-[var(--color-foreground)] font-mono-data">${balance.toLocaleString()}</p>
            {target && (
              <div className="mt-3">
                <div className="flex justify-between text-xs text-[color:rgba(246,212,203,0.64)] mb-1">
          ...
          <div className="card-civic">
            <p className="text-xs text-[color:rgba(246,212,203,0.64)] mb-1">30-Day Inflows</p>
            <div className="flex items-center gap-2">
              <ArrowDown className="w-4 h-4 text-[var(--color-institutional)]" />
              <p className="text-2xl font-semibold text-[var(--color-institutional)] font-mono-data">${inflows.toLocaleString()}</p>
            </div>
          </div>
          <div className="card-civic">
            <p className="text-xs text-[color:rgba(246,212,203,0.64)] mb-1">30-Day Outflows</p>
            <div className="flex items-center gap-2">
          ...
          <div className="card-civic">
            <p className="text-xs text-[color:rgba(246,212,203,0.64)] mb-1">Net Flow (30d)</p>
            <div className="flex items-center gap-2">
              {net >= 0 ? <TrendingUp className="w-4 h-4 text-[var(--color-institutional)]" /> : <TrendingDown className="w-4 h-4 text-[var(--color-accent)]" />}
              <p className="text-2xl font-semibold font-mono-data" style={{ color: net >= 0 ? 'var(--color-institutional)' : 'var(--color-accent)' }}>
        ...
        <section>
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-2">
              <Shield className="w-4 h-4 text-[var(--color-institutional)]" />
              <h2 className="text-xl font-semibold text-[var(--color-foreground)]" style={{ fontFamily: 'var(--anu-type-display)' }}>
                Ledger Entries
              </h2>
            </div>
            <span className="text-xs text-[color:rgba(246,212,203,0.78)] bg-[color:rgba(224,177,21,0.2)] px-2.5 py-1 rounded-full">
              Append-only
            </span>
          </div>
          ...
          <div className="card-civic p-0 overflow-hidden">
            {ledgerLoading ? (
              ...
              <>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-[var(--color-border)] bg-[var(--color-muted)]">
                        <th className="px-4 py-3 text-left text-xs font-medium text-[color:rgba(246,212,203,0.68)] uppercase tracking-wider">Date</th>
                        <th className="px-4 py-3 text-left text-xs font-medium text-[color:rgba(246,212,203,0.68)] uppercase tracking-wider">Type</th>
                        <th className="px-4 py-3 text-right text-xs font-medium text-[color:rgba(246,212,203,0.68)] uppercase tracking-wider">Amount</th>
                        <th className="px-4 py-3 text-left text-xs font-medium text-[color:rgba(246,212,203,0.68)] uppercase tracking-wider">Description</th>
                      </tr>
                    </thead>
                    <tbody>
                      {ledgerEntries.map((entry) => (
                        <tr key={entry.id} className="border-b border-[var(--color-border)] hover:bg-[color:rgba(246,212,203,0.06)]">
                          <td className="px-4 py-3 text-[color:rgba(246,212,203,0.78)] font-mono-data text-xs">
                          ...
                          <td className="px-4 py-3 text-[color:rgba(246,212,203,0.78)] text-xs max-w-xs truncate">
                            {entry.description || '—'}
                          </td>
                        </tr>
                      ))}
                ...
                {ledgerPages > 1 && (
                  <div className="flex items-center justify-between px-4 py-3 border-t border-[var(--color-border)]">
                    ...
                    <span className="text-xs text-[color:rgba(246,212,203,0.78)] font-mono-data">
              ...
              <div className="p-6 text-center text-[color:rgba(246,212,203,0.78)]">
        ...
        <div className="mt-10 p-4 rounded-xl border border-[var(--color-border)] bg-[var(--color-muted)]">
          <p className="text-sm text-[color:rgba(246,212,203,0.82)] text-center">
            <strong className="text-[var(--color-foreground)]">Append-only ledger:</strong> Entries cannot be modified or deleted.
