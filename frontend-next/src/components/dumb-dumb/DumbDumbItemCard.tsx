...
export function DumbDumbItemCard({ item, href, onBuy, actionLabel = 'Fund this instead' }: DumbDumbItemCardProps) {
  const content = (
    <Card
      padding="lg"
      className="group relative h-full overflow-hidden border-[var(--color-border)] bg-[linear-gradient(180deg,rgba(246,212,203,0.97),rgba(246,212,203,0.96))] shadow-[0_18px_36px_-28px_rgba(30,2,39,0.45)]"
    >
      <div className="pointer-events-none absolute inset-x-6 top-0 h-px bg-[linear-gradient(90deg,transparent,rgba(30,2,39,0.55),transparent)]" />
      ...
      <div className="mt-6 flex items-end justify-between gap-4">
        ...
        <div className="rounded-full border border-[rgba(124,65,60,0.18)] bg-[var(--color-sage-light)] px-3 py-1.5 text-xs font-semibold text-[var(--color-forest)]">
          {item.destination_pool.name}
        </div>
      </div>

      <div className="mt-6 rounded-[1.35rem] border border-[rgba(124,65,60,0.14)] bg-[linear-gradient(180deg,rgba(246,212,203,0.88),rgba(246,212,203,0.92))] p-4">
      ...
      <div className="mt-6 flex items-center justify-between gap-3">
        ...
        {onBuy ? (
          ...
          <Link
            href={href}
            className="inline-flex items-center justify-center gap-2 rounded-full bg-[var(--color-forest)] px-4 py-2 text-sm font-medium text-[var(--color-foreground)] transition-all duration-300 hover:brightness-95"
          >
            {actionLabel}
