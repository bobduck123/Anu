...
function renderWishlistFigure(item: DumbDumbItem) {
  if (item.image_url) {
    return (
      <img
        src={item.image_url}
        alt={item.title}
        className="mx-auto h-[220px] w-auto max-w-full object-contain drop-shadow-[0_24px_28px_rgba(29,22,15,0.18)] md:h-[260px]"
      />
    );
  }
  ...
  return (
    <div className="flex h-[220px] items-center justify-center md:h-[260px]">
      <div className="flex h-32 w-32 items-center justify-center rounded-[2.2rem] border border-[rgba(42,33,24,0.18)] bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.98),rgba(227,220,209,0.96))] text-6xl shadow-[0_24px_24px_rgba(29,22,15,0.12)]">
        {dumbDumbVisual(item)}
      </div>
    </div>
  );
    ...
    >
      ...
      <div className={`mt-4 flex ${variant.noteClass}`}>
        <div className="max-w-[220px]">
          <div
            className="text-[1.9rem] leading-[1.02] text-[rgba(28,24,20,0.92)]"
          ...
          <div className="mt-2 h-px w-full max-w-[180px] bg-[rgba(38,31,25,0.5)]" />
          <div className="mt-2 flex flex-wrap items-center gap-2 text-[11px] uppercase tracking-[0.18em] text-[rgba(56,48,39,0.7)]">
            <span>{item.source_site_name || brand.name}</span>
            <span>{formatMoney(item.price_cents, item.currency)}</span>
          </div>
        </div>
      </div>
      <div className="pointer-events-none absolute inset-x-8 bottom-2 flex items-center justify-between text-[11px] uppercase tracking-[0.18em] text-[rgba(57,50,43,0.56)] opacity-0 transition-opacity duration-300 group-hover:opacity-100">
...
export function DumbDumbWishlistScreen({ list }: DumbDumbWishlistScreenProps) {
  ...
  return (
    <div className="min-h-screen bg-[#d7d2c9] px-3 py-4 text-[#211d18] md:px-6 md:py-8">
      <div className="mx-auto max-w-[980px] rounded-[2.25rem] border border-[rgba(46,38,29,0.12)] bg-[#ebe6dd] p-3 shadow-[0_30px_70px_rgba(44,34,20,0.12)] md:p-5">
        <div
          className="relative overflow-hidden rounded-[2rem] border border-[rgba(59,49,39,0.12)] bg-[#ece8df] px-4 py-8 md:px-10 md:py-10"
          style={{
            backgroundImage:
              'linear-gradient(rgba(55,45,36,0.12) 1px, transparent 1px), linear-gradient(90deg, rgba(55,45,36,0.12) 1px, transparent 1px)',
            backgroundSize: '39px 39px',
          }}
        >
          <div className="pointer-events-none absolute -left-6 top-12 h-40 w-72 rounded-full border border-[rgba(27,22,16,0.8)]" />
          <div className="pointer-events-none absolute left-14 top-0 h-32 w-60 rounded-full border border-[rgba(27,22,16,0.8)]" />
          <div className="pointer-events-none absolute left-12 top-6 h-20 w-40 rounded-full border border-[rgba(27,22,16,0.8)]" />

          <div className="relative">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="rounded-full border border-[rgba(36,31,24,0.14)] bg-[rgba(255,255,255,0.62)] px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-[rgba(44,36,28,0.78)]">
                {brand.wishlistHostLabel}
              </div>
              <Link
                href={`/dumb-dumb/${list.slug}`}
                className="inline-flex items-center gap-2 rounded-full border border-[rgba(36,31,24,0.16)] bg-[rgba(255,255,255,0.66)] px-4 py-2 text-xs font-semibold uppercase tracking-[0.16em] text-[rgba(34,29,23,0.84)] transition-colors hover:bg-white"
              >
                Open platform list
                <ArrowUpRight className="h-4 w-4" />
              </Link>
            ...
            <div className="pt-6 text-center">
              ...
              <p className="mx-auto mt-4 max-w-3xl text-sm leading-7 text-[rgba(44,36,28,0.78)] md:text-base">
        ...
        <div className="grid gap-4 px-1 pb-1 pt-5 md:grid-cols-[1.05fr_0.95fr]">
          <div className="rounded-[1.6rem] border border-[rgba(58,47,35,0.12)] bg-[rgba(255,255,255,0.72)] p-5">
            <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[rgba(54,45,36,0.64)]">Transparent note</p>
            <p className="mt-3 text-sm leading-7 text-[rgba(35,30,24,0.82)]">
              {list.parody_disclaimer}
            </p>
          </div>

          <div className="rounded-[1.6rem] border border-[rgba(45,90,61,0.18)] bg-[linear-gradient(180deg,rgba(233,239,229,0.92),rgba(255,255,255,0.88))] p-5">
            ...
            <div className="mt-4 space-y-3">
              {items.map((item) => (
                <div
                  key={`impact-${item.id}`}
                  className="rounded-[1.2rem] border border-[rgba(45,90,61,0.12)] bg-white/82 px-4 py-3"
                >
