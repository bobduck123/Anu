...
export function StartHereRail() {
  return (
    <AnuSurfacePanel tone="soft" className="relative overflow-hidden p-5 md:p-7">
      <span className="pointer-events-none absolute -right-20 -top-16 h-52 w-52 rounded-full bg-[#f6d4cb]/22 blur-3xl" />
          ...
          {STEPS.map((step, index) => {
            ...
            return (
              <Link key={step.title} href={step.href} className="group block">
                <AnuSurfacePanel
                  tone="quiet"
                  className="flex h-full flex-col gap-4 p-4 transition-transform duration-300 group-hover:-translate-y-0.5 group-hover:border-[color:rgba(246,212,203,0.2)]"
                >
                  <div className="flex items-start justify-between gap-3">
                    <AnuChip tone="accent">{`Step ${index + 1}`}</AnuChip>
                    <span className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-[var(--anu-border-signal)] bg-[#1e0227] text-[#f6d4cb]">
                      <Icon className="h-4 w-4" />
                    </span>
                  </div>

                  <div>
                    <p className="text-base font-semibold text-[var(--color-foreground)]">{step.title}</p>
                    <p className="mt-2 text-sm leading-6 text-[color:rgba(246,212,203,0.84)]">{step.description}</p>
                  </div>

                  <div className="mt-auto inline-flex items-center gap-1 text-xs font-semibold text-[#f6d4cb]">
                    Open step
