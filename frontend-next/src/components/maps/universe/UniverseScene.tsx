...
export function UniverseScene({
  ...
  return (
    <div className="relative h-[30rem] w-full overflow-hidden rounded-[1.75rem] border border-slate-800 bg-[#02060d] md:h-[38rem]">
      <QuantumCanvas
        ref={quantumRef}
        data={quantumData}
      ...
      <div className="pointer-events-none absolute inset-x-4 bottom-4 z-10 flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="rounded-full border border-white/10 bg-black/35 px-4 py-2 text-xs text-slate-300 shadow-[0_16px_40px_rgba(0,0,0,0.3)] backdrop-blur-xl">
          Drag to orbit / scroll to zoom / select a star to sync the explainer
        </div>
      </div>
    </div>
  );
