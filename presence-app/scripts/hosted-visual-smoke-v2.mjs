...
// ── 1. Public desktop /p/ ──
{
  ...
  const hasCta = text.toLowerCase().includes("begin") || text.toLowerCase().includes("conversation");
...
// ── 4. Owner editor ──
{
  ...
  if (!EMAIL || !PASSWORD) {
    ...
    const hasDarkCockpit = await page.locator("body").evaluate((b) => b.innerHTML.includes("v2-cockpit"));
    const url = page.url();
    ...
    // Try to capture owner preview
    try {
      await page.goto(`/studio/${ROOM_ID}/editor/preview`, { waitUntil: "networkidle" });
      await page.waitForTimeout(4000);
      const previewHtml = await page.content();
      const previewHasV2 = previewHtml.includes("presence-studio-v2-public");
      const previewHasDraftLabel = previewHtml.includes("Draft preview");
      await page.screenshot({ path: `${OUT}/hosted-room11-owner-preview.png`, fullPage: false });
      log("OWNER_PREVIEW", {
        v2_public: previewHasV2,
        draft_label: previewHasDraftLabel,
      });
    } catch (e) {
      log("OWNER_PREVIEW", { error: e.message });
    }
...
// ── 5. Legacy negative (Room 1) ──
{
  ...
  if (!EMAIL || !PASSWORD) {
    ...
    await page.goto("/studio/1/editor", { waitUntil: "networkidle" });
...
for (const r of results) {
  const status = Object.entries(r).every(([k, v]) => k === "stage" || k === "errors" || k === "skipped" || k === "error" || v !== false && v !== 0) ? "PASS" : "CHECK";
