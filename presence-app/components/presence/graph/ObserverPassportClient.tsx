      ...
      {items.length === 0 ? (
        <section className="rounded-3xl border border-stone-800 bg-stone-900 p-6">
          ...
          <p className="mt-3 text-sm leading-6 text-stone-300">
            Enter Rooms, save what matters, and your path through the world will begin to appear.
          </p>
          <Link href="/gallery" className="mt-5 inline-flex items-center gap-2 rounded-2xl bg-orange-300 px-5 py-3 text-sm font-semibold text-stone-950">
            Find Rooms
            <ArrowRight className="h-4 w-4" />
          </Link>
        </section>
          ...
          {Object.entries(groups).map(([label, stamps]) => (
            <section key={label} className="rounded-3xl border border-stone-800 bg-stone-900 p-5">
              ...
              <div className="mt-4 grid gap-3">
                {stamps.map((stamp) => (
                  <article key={stamp.id} className="flex items-start justify-between gap-4 rounded-2xl bg-stone-950/60 p-4">
                    <div>
                      <p className="font-medium text-stone-50">{stamp.label || stamp.stamp_type.replace(/_/g, " ")}</p>
                      <p className="mt-1 text-xs uppercase tracking-[0.16em] text-stone-500">{stamp.stamp_type}</p>
                    </div>
