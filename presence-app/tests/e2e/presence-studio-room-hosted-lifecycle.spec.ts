...
test.describe("hosted Studio Room owner lifecycle", () => {
  test.skip(!hostedGate, "Set PRESENCE_HOSTED_SMOKE=1 to run the hosted owner lifecycle smoke.");
  test.skip(
    hostedGate && missingEnv.length > 0,
    `Missing hosted smoke env vars: ${missingEnv.join(", ")}`,
  );

  test("real owner creates, edits, saves, reloads, and keeps a TemplateKit draft private", async ({
    page,
    context,
    request,
  }) => {
    const apiBase = required("PRESENCE_E2E_API_URL");
    const marker = `Hosted lifecycle archive proof ${Date.now()}`;
    ...
    await page.getByTestId(`template-kit-select-${primaryHostedKit.id}`).click();
    ...
    expect(createdBody.data).toMatchObject({
      template_kit_id: primaryHostedKit.id,
      template_kit_name: primaryHostedKit.name,
    ...
    await expect(
      page.locator(`[data-testid="studio-room-canvas"][data-template-kit-id="${primaryHostedKit.id}"]`),
    ).toBeVisible();
