...
test("Studio V3 local changes do not alter BBB public output or public routes", async ({ page, request }) => {
  ...
  await page.screenshot({ path: `${evidenceDir}/03-public-before-local-v3.png`, fullPage: true });
  ...
  await page.getByTestId("presence-studio-v3-test-visitor").click();
  await page.screenshot({ path: `${evidenceDir}/04-test-as-visitor-local-only.png`, fullPage: true });
