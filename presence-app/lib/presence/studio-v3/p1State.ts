...
import { isStudioV3CollectionSourceRef, isStudioV3PlacementId, isStudioV3SourceRef } from "./sourceRefs.ts";
...
export interface StudioV3StructuralStageBlocked {
  status: "blocked";
  reason: "room-missing" | "look-missing";
  originalDocument: StudioV3Document;
  stagedDocument: StudioV3Document;
  savepoint: StudioV3StructuralSavepoint;
  compare: {
...
export function restoreStudioV3Savepoint(
  ...
  const currentPieceRefs = new Set(Object.keys(current.pieces));
  ...
  for (const saved of [...savepoint.rooms].sort((a, b) => a.order - b.order)) {
    ...
    const placements = saved.placements.filter((placement) => {
      if (currentPieceRefs.has(placement.sourceRef)) return true;
      issues.push({ kind: "missing-piece", reference: placement.sourceRef });
      return false;
    }).map((placement) => normalizePrivatePlacement(current, { ...placement }));
    const knownObjectIds = new Set(Object.values(current.pieces).map((piece) => piece.id));
    const baseObjectIds = saved.baseObjectIds.filter((objectId) => {
      if (knownObjectIds.has(objectId)) return true;
  ...
  const scopeExists = (scopeKind: StudioV3ScopeKind, scopeId: string) => {
    if (scopeKind === "presence") return true;
    if (scopeKind === "room") return restoredRoomIds.has(scopeId);
    if (scopeKind === "collection") return Boolean(current.collections[scopeId]);
    return Boolean(current.pieces[scopeId]) || Object.values(current.pieces).some((piece) => piece.id === scopeId);
  };
...
export function restoreStudioV3Metadata(
  ...
  const namedLooks = parseNamedLooks(source.named_looks);
  const namedLookMap = Object.fromEntries(namedLooks.map((look) => [look.id, look]));
  const parsedSavepoints = source.savepoints.map((row) => parseSavepoint(row));
  if (parsedSavepoints.some((savepoint) => !savepoint)) {
    return { document: baseDocument, report: { status: "rejected", issues: [] } };
  }
  document = {
    ...document,
    mode: source.owner_mode,
    namedLooks,
  ...
  for (const row of source.placements) {
    ...
    if (!document.pieces[parsedPlacement.sourceRef]) {
  ...
  let rooms = document.rooms.map((room) => ({
    ...room,
    placements: (placementsByRoom.get(room.id) ?? room.placements).sort((a, b) => a.order - b.order),
  }));
...
function remapRoomForStyle(
  ...
  const accounting: StudioV3StructuralAccounting[] = [];
  ...
  for (const candidate of candidates) {
    const piece = candidate.placement
      ? document.pieces[candidate.placement.sourceRef]
      : Object.values(document.pieces).find((item) => item.id === candidate.objectId);
    ...
    let zone = compatibleZones.find((item) => item.maxObjects === undefined || (zoneCounts.get(item.id) ?? 0) < item.maxObjects);
    if (piece && (piece.sourceStatus !== "current" || candidate.placement?.visibility === "hidden")) {
      compatible = false;
      outcome = "shelved";
      zone = undefined;
    ...
    if (candidate.placement) {
      candidate.placement.status = outcome === "unplaced" || outcome === "shelved" ? "shelved" : outcome;
      candidate.placement.reason = outcome === "placed" ? undefined : reason;
    }
    const beforeZoneId = beforeZones.get(candidate.objectId);
    const afterZoneId = zone?.id;
    const afterStatus = candidate.placement?.status ?? outcome;
    accounting.push({
      objectId: candidate.objectId,
      sourceRef,
      beforeStatus,
...
function projectCompatibility(document: StudioV3Document): Array<Record<string, unknown>> {
  return document.rooms.flatMap((room) => {
    const baseRows = room.baseObjectIds.flatMap((objectId) => {
      const piece = Object.values(document.pieces).find((candidate) => candidate.id === objectId);
      if (!piece) return [];
      return [{
        sourceRef: piece.sourceRef,
        roomId: room.id,
        roomStyleId: room.styleId,
        status: piece.compatibleRoomStyles.includes(room.styleId) ? "compatible" : "incompatible",
        reasonCode: piece.compatibleRoomStyles.includes(room.styleId) ? "supported-piece" : "unsupported-piece",
      }];
    });
    const placementRows = room.placements.map((placement) => ({
      sourceRef: placement.sourceRef,
      roomId: room.id,
      roomStyleId: room.styleId,
      status: placement.status === "placed"
        ...
        : placement.status === "incompatible"
          ...
          : placement.status === "shelved"
            ? "shelved"
            : "unresolved",
      reasonCode: placement.reason ? reasonCode(placement.reason) : "supported-piece",
    }));
...
export function isSafeStudioV3MetadataEnvelope(value: unknown): value is StudioV3PrivateMetadata {
  if (!isRecord(value) || hasForbiddenMetadataValue(value)) return false;
  const allowed = new Set(["owner_mode", "named_looks", "layer_locks", "savepoints", "placements", "restore", "compatibility"]);
  if (Object.keys(value).some((key) => !allowed.has(key))) return false;
  if (value.owner_mode !== "simple" && value.owner_mode !== "advanced-creative") return false;
  if (!Array.isArray(value.named_looks) || !Array.isArray(value.layer_locks) || !Array.isArray(value.savepoints)
    || !Array.isArray(value.placements) || !Array.isArray(value.compatibility) || !isRecord(value.restore)) return false;
  if (parseNamedLooks(value.named_looks.filter(isRecord)).length !== value.named_looks.length) return false;
  if (!value.layer_locks.every((row) => isRecord(row) && Boolean(parseLayerLock(row)))) return false;
  if (!value.savepoints.every((row) => isRecord(row) && Boolean(parseSavepoint(row)))) return false;
  ...
  if (!isSafeRestoreMetadata(value.restore)) return false;
  if (!value.compatibility.every(isSafeCompatibilityRow)) return false;
  return true;
}
...
function hasForbiddenMetadataValue(value: unknown, key = ""): boolean {
  ...
  if (typeof value === "string") {
    return /(?:https?:\/\/|data:|blob:|file:|javascript:|\bggm\b|[^@\s]+@[^@\s]+\.[^@\s]+)/i.test(value);
  }
...
function parseNamedLooks(rows: Array<Record<string, unknown>>): StudioV3Look[] {
  return rows.flatMap((row) => {
    if (!hasExactKeys(row, ["id", "name", "baseLookId", "values", "provenance", "createdAt", "updatedAt"], ["id", "name", "values", "provenance"])
      || typeof row.id !== "string" || !/^named:[A-Za-z0-9_.:-]{1,154}$/.test(row.id) || typeof row.name !== "string" || !row.name.trim()
      || row.name.length > 160 || typeof row.provenance !== "string" || !isStableReference(row.provenance) || !isRecord(row.values)
...
function parseSavepoint(value: Record<string, unknown>): StudioV3StructuralSavepoint | null {
  ...
  if (new Set(parsedLocks.map((lock) => lock.id)).size !== parsedLocks.length) return null;
...
function parseSavepointRequiredCta(
  ...
  if (value.sourceRef !== undefined && (typeof value.sourceRef !== "string" || !isStudioV3SourceRef(value.sourceRef))) return null;
