...
def _degraded_node_summary(node_slug: str, node_name: str, reason: str):
    return ok({
        "status": "degraded",
        "data_mode": "fallback",
        "reason": reason,
        "node": {"slug": node_slug, "name": node_name},
        "totals": {
            "inflows_30d": 0,
            "outflows_30d": 0,
            "admin_ratio_30d": 0.0,
...
def node_summary():
    ...
    fallback_slug = node_param or current_app.config.get("DEFAULT_NODE_SLUG") or "default-node"
    fallback_name = "Default Node"
    ...
    try:
        ...
        current_app.logger.exception("Public transparency node summary degraded")
        try:
            db.session.rollback()
        except Exception:
            pass
        return _degraded_node_summary(
            node_slug=fallback_slug,
            node_name=fallback_name,
            reason="transparency_data_unavailable",
        )
